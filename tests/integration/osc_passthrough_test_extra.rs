...
async fn test_large_packet_handling() {
    ...
    sender_socket
        .send_to(&large_msg, "127.0.0.1:19029")
        .await
        .unwrap();
...
async fn test_invalid_packet_ordering() {
    ...
    sender_socket
        .send_to(&valid_msg1, "127.0.0.1:19030")
        .await
        .unwrap();
    sleep(Duration::from_millis(10)).await;

    sender_socket
        .send_to(b"INVALID PACKET", "127.0.0.1:19030")
        .await
        .unwrap();
    sleep(Duration::from_millis(10)).await;
    ...
    sender_socket
        .send_to(&valid_msg2, "127.0.0.1:19030")
        .await
        .unwrap();
    sleep(Duration::from_millis(10)).await;

    sender_socket
        .send_to(&[0xFF, 0xFF, 0xFF], "127.0.0.1:19030")
        .await
        .unwrap();
    sleep(Duration::from_millis(10)).await;
    ...
    sender_socket
        .send_to(&valid_msg3, "127.0.0.1:19030")
        .await
        .unwrap();
    ...
    assert_eq!(
        received_values,
        vec![1, 2, 3],
        "Should receive all valid messages in order"
    );
...
async fn test_empty_packet_handling() {
    ...
    let valid_msg = create_test_message(
        "/test/after_empty",
        vec![OscType::String("test".to_string())],
    );
    sender_socket
        .send_to(&valid_msg, "127.0.0.1:19031")
        .await
        .unwrap();
